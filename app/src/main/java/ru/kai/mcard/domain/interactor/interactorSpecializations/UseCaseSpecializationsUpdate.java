...
public class UseCaseSpecializationsUpdate extends BaseUseCase<Integer, ParamsSpecialization> {
    private final ISpecializationRepository repository;

    @Inject
    public UseCaseSpecializationsUpdate(ISpecializationRepository repository){
        this.repository = repository;
    }

    @Override
    public Flowable buildUseCaseFlowable(ParamsSpecialization paramsSpecialization) {
        return this.repository.updateSpecialization(paramsSpecialization.getDModel());
    }
