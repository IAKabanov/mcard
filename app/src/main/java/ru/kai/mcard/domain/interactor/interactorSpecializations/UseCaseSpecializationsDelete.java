...
public class UseCaseSpecializationsDelete extends BaseUseCase<Integer, ParamsSpecialization> {
    private final ISpecializationRepository repository;

    @Inject
    public UseCaseSpecializationsDelete(ISpecializationRepository repository){
        this.repository = repository;
    }

    @Override
    public Flowable buildUseCaseFlowable(ParamsSpecialization paramsSpecialization) {
        return this.repository.deleteSpecialization(paramsSpecialization.getDModel());
    }
